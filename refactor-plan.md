# Refactoring Plan: Unify API Request Handling

This document outlines the step-by-step plan to refactor the API request handling mechanism. The goal is to centralize data unpacking and error handling within `src/utils/request.ts`, simplifying business logic in stores and components.

---

## Step 1: Modify `src/utils/request.ts`

**File:** [`src/utils/request.ts`](src/utils/request.ts)

**Action:** Replace the entire content of the file with the code below. This re-introduces the `api` wrapper function from `request.old.ts` and adjusts the exports. The `axios` instance will no longer be the default export.
...
## Step 2: Modify API Layer Files

**Directory:** [`src/api/`](src/api/)

**Action:** For every `.ts` file in this directory, perform the following changes.

1.  **Change Import:**
    *   **Find:** `import request from '@/utils/request';`
    *   **Replace with:** `import { api } from '@/utils/request';`

2.  **Change Function Body:**
    *   **Find:** `return request({ url: '...', data });`
    *   **Replace with:** `return api('...', data);`
    *   The `async` keyword on functions can be removed as we are now directly returning the promise from `api`.

**Example for `src/api/staff.api.ts`:**
...
export const getStaff = (data?: StaffListQueryDTO): Promise<StaffProfile[]> => {
    return api('/user/staff', data);
};
// ...
```

**Affected Files:**
*   [`src/api/aircraft.api.ts`](src/api/aircraft.api.ts)
*   [`src/api/airport.api.ts`](src/api/airport.api.ts)
*   [`src/api/flight.api.ts`](src/api/flight.api.ts)
*   [`src/api/permission.api.ts`](src/api/permission.api.ts)
*   [`src/api/pilot.api.ts`](src/api/pilot.api.ts)
*   [`src/api/resource.api.ts`](src/api/resource.api.ts)
*   [`src/api/role.api.ts`](src/api/role.api.ts)
*   [`src/api/sms.api.ts`](src/api/sms.api.ts)
*   [`src/api/staff.api.ts`](src/api/staff.api.ts)
*   [`src/api/statistics.api.ts`](src/api/statistics.api.ts)
*   [`src/api/user.api.ts`](src/api/user.api.ts)

---

## Step 3: Modify Store Layer Files

**Directory:** [`src/store/`](src/store/)

**Action:** For every `.ts` file in this directory, remove the manual data unpacking and error handling logic. The API calls will now return the final data directly.

**Example for `src/store/user.store.ts`:**

**File:** [`src/store/user.store.ts`](src/store/user.store.ts)

1.  **Modify `myself` function:**
    *   **Find:**
        ```typescript
    ...
    *   **Replace with:**
        ...
        const myself = async (refresh = false) => {
            try {
                const initData = await initUser(); // initUser now returns the unpacked data: { user: UserItem, ... }
                const mySelf = initData.user as UserItem;
                console.log('mySelf', mySelf, self.value, refresh);
                if (mySelf?.id) {
                    self.value = mySelf;
                ...
                console.error('初始化用户信息或获取权限失败:', error);
                clearPermissions();
            }
            return self.value;
        };
...
2.  **Modify `fetchStaff` function:**
    ...
    *   **Replace with:**
        ...
        const fetchStaff = async () => {
            ...
            try {
                if (!Object.keys(staff.value).length) {
                    const res = await getStaffList(); // Now directly returns UserItem[]
                    const obj: Record<string, UserItem> = {};
                    if (res && res.length) {
                        for (let user of res) {
                            obj[user.userId] = user;
...
**Affected Files:**
*   [`src/store/aircraft.store.ts`](src/store/aircraft.store.ts)
*   [`src/store/airport.store.ts`](src/store/airport.store.ts)
*   [`src/store/department.store.ts`](src/store/department.store.ts)
*   [`src/store/pilot.store.ts`](src/store/pilot.store.ts)
*   [`src/store/user.store.ts`](src/store/user.store.ts)

---

## Step 4: Modify Page/Component Layer Files

**Directory:** [`src/pages/`](src/pages/)

**Action:** Search for any `.vue` files that import from ` '@/api/...' ` and perform modifications similar to the store files.

**Example for `src/pages/staff/duty.vue`:**

**File:** [`src/pages/staff/duty.vue`](src/pages/staff/duty.vue)

1.  **Modify `fetchDutyData` function:**
    *   **Find:**
        ```typescript
    ...
    *   **Replace with:**
        ...
        const fetchDutyData = async () => {
            try {
                // API call now returns the data directly
                const res = await getDutyToday();
                dutyData.value = res || [];
            } catch (error) {
                console.error('获取值班数据失败:', error);
                // Handle error state in UI, e.g., clear data or show a message
                dutyData.value = [];
            }
        };
        ```

**Recommendation:** Perform a global search in the `src/pages` directory for the string `from '@/api/` to identify all components that need to be updated.

---

## Step 5: Final Review and Testing

After applying all the changes above, the project should be thoroughly tested.

1.  **Build:** Ensure the project compiles without any type errors.
2.  **Runtime Testing:**
    *   Navigate through all pages of the application.
    *   Verify that data is being loaded and displayed correctly.
    *   Test scenarios where API calls might fail (e.g., by disconnecting from the network) to ensure that error messages are displayed correctly and the application remains stable.
    *   Test token refresh logic by simulating a 401 error.
    *   Verify that loading indicators appear and disappear as expected.

This concludes the detailed refactoring plan.