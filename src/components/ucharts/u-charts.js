...
function getTouches(touches, opts, e) {
    ...
    // 检查 touches 对象是否有 clientX 属性
    if (touches.clientX) {
        // 如果 opts.rotate 为真，说明图表旋转了
        if (opts.rotate) {
            // 计算 y 坐标，考虑旋转情况
            ...
            console.warn(
                ...
                '图表高度', opts.height,
                '图表分辨率', opts.pix,
                '修正', (opts.height / opts.pix / 2) * (opts.pix - 1) * opts.pix,
                '原始', touches.pageY - e.currentTarget.offsetTop,
                'u结果', (touches.pageY - e.currentTarget.offsetTop - (opts.height / opts.pix / 2) * (opts.pix - 1)) * opts.pix,
                "x", touches.clientX 
            );
            // 如果 opts.rotate 为假，说明图表没有旋转
            // 计算 x 坐标
            x = touches.clientX * opts.pix;
            // 计算 y 坐标
            // 根据点测得知touches.pageY - e.currentTarget.offsetTop ,就是再px上的实际位置,但是害的转换成高分屏.
            y = (touches.pageY - e.currentTarget.offsetTop - (opts.height / opts.pix / 2) * (opts.pix - 1)) * opts.pix;
            // y = (touches.pageY - e.currentTarget.offsetTop ) * opts.pix;
        }
    } else {
        // 如果 touches 对象没有 clientX 属性，可能是触摸事件对象
        if (opts.rotate) {
            // 计算 y 坐标，考虑旋转情况
...
function findCurrentIndex(currentPoints, calPoints, opts, config) {
    ...
    if (calPoints && calPoints.length > 0) {
        ...
        if (!opts.categories) {
            ...
            // 如果图表类型为线图或面积图且边界对齐方式为'justify'，使用原始X轴刻度点
...
function isInExactChartArea(currentPoints, opts, config) {
    console.warn(
        '函数isInExactChartArea:', 'opts.area就是设定的padding*像素比',
        'x-left:', currentPoints.x >= opts.area[3] - 10,
        'x-right:', currentPoints.x <= opts.width - opts.area[1] + 10,
        'y-top:', currentPoints.y >= opts.area[0],
        'y-bottom:', currentPoints.y <= opts.height - opts.area[2]
    )
    // 确保触摸点的x坐标小于等于图表宽度减去右侧边距加上10。
    return currentPoints.x <= opts.width - opts.area[1] + 10 &&
        // 确保触摸点的x坐标大于等于左侧边距减去10。
        currentPoints.x >= opts.area[3] - 10 &&
        // 确保触摸点的y坐标大于等于顶部边距。
        currentPoints.y >= opts.area[0] &&
        // 确保触摸点的y坐标小于等于图表高度减去底部边距。
...
uCharts.prototype.getCurrentDataIndex = function (e) {
    ...
    // 检查触摸事件对象
    if (e.changedTouches) {
        touches = e.changedTouches[0]; // 获取触摸点信息
    } else {
        touches = e.mp.changedTouches[0]; // 兼容某些框架的触摸点信息
    }
    ...
    if (touches) {
        ...
        console.warn('______________', _touches$)
        // 根据图表类型查找当前数据索引
