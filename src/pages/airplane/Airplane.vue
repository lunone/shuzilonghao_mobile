<template>
    ...
    <van-tabs v-model:active="activeTab" @change="sortChanged">
        <template v-for="aircraftGroup, groupName in aircraftSorted" :key=" groupName">
            <van-tab :name="groupName" :title="`${aircraftGroup.name}(${aircraftGroup.aircrafts.length})`"
                v-if="aircraftGroup.aircrafts.length > 0">
                <div class="aircrafts">
                    <div class="buttons">
                        <van-button v-for="aircraft in aircraftGroup.aircrafts" hairline size="normal"
                            :type="selectedAircraft?.acReg == aircraft.acReg ? 'primary' : `default`"
                            @click="showInfo(aircraft)" :key="aircraft.acReg">
                            {{ aircraft.acReg }}
                        </van-button>
                    </div>
                    <van-tabs v-model:active="infoTab" :lazy-render="false">
                        <van-tab name="detail" title="飞机参数">
                            <div class="detail" v-if="selectedAircraft">
                                <div class="mate">
                                    <div class="name">
                                        <span class="id">{{ selectedAircraft.acReg }}</span>
                                        <span class="type">({{ selectedAircraft.acType }})</span>
                                    </div>
                                </div>
                                <div class="kv">
                                    <!-- 退役的飞机要显示退役日期 -->
                                    <div v-if="groupName == 'retired'">{{ selectedAircraft.endDate }} 退役</div>
                                    <!-- 默认显示的 -->
                                    <template v-for="flag in [true, false]">
                                        <template v-for="showOBj in aircraftKeysArray" :key="showOBj.key">
                                            <div v-show="flag || showMoreFlags[selectedAircraft.acReg]" class="item">
                                                <span class="key">{{ showOBj.name }}</span>
                                                <span class="value">
                                                    {{ _.attempt(showOBj.func || (_ => _),
                                                        selectedAircraft[showOBj.key]) }}
                                                    {{ showOBj.unit || '' }}
                                                </span>
                                            </div>
                                        </template>
                        ...
                        <van-tab name="mel" :title="`保留单详情(${mels?.length || 0})`">
...
import MelCardVue from '../maintenance/MelCard.vue';
...
const aircraftKeysArray: { key: keyof AircraftItem, name: string, unit?: string, default: boolean, func?: (...args: any[]) => unknown }[] = [
    ...
    { key: 'engineModel', name: '发动机型号', default: true },
...
.aircrafts {
    .buttons {
        ...
        //     &.van-button--primary.van-button--normal.van-button--plain.van-button--active {
